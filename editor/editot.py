...
saveAs_file_icon = PhotoImage(file='icons/savAs.png')
...
close_icon = PhotoImage(file='icons/exit_editor.png')
...
for j in ('cut', 'copy', 'paste', 'undo', 'redo'):
    cmd = eval(j)
    popup_menu.add_command(label=j, compound='left', command=cmd)
